    ...
    client.get(url, new AsyncHttpResponseHandler() {
      ...
      public void onSuccess(int statusCode, Header[] headers, byte[] response) {
        ...
        for (int i = 1; i < items.size(); i++) {
          formattedRows
            .add(Arrays.asList(items.get(i).split(",(?=([^\\\"]*\\\"[^\\\"]*\\\")*[^\\\"]*$)")));
        }
        ...
        for (int i = 0; i < formattedRows.size(); i++) {
          ...
          DataModel allData = new DataModel(indV.get(1), "as of " + indV.get(2), indV.get(3),
            indV.get(4), indV.get(5), indV.get(0));
        ...
        map.entrySet().stream().forEach(e -> {
            ...
            DataModel allData = new DataModel(e.getKey(), "as of today", String.valueOf(ccs),
              String.valueOf(deaths), String.valueOf(recovered), "• All regions");
